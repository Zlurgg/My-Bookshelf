...
**Status:** Pending — runs after the Google Books integration branch is merged to `main`
**Goal:** make the Google Books–integrated build safe to promote to a Play Store closed-testing track
**Related plans:** `google-books-followups.md` (deferred work that isn't release-blocking)

## Purpose

The Google Books integration work landed on `main` from `spike-test-google-books`. This plan is the bridge from "merged" to "promoted to a closed-testing track." Most of the work here is release engineering and observability, not application code.

## Prerequisites — code state at merge

Before starting this plan, verify the following code state on `main`. All of these landed on `spike-test-google-books` before merge; this checklist exists so the next session can confirm nothing regressed during the merge.

- [ ] `DataError.Remote.PROVIDER_UNAVAILABLE` exists and is emitted by `GoogleBooksRemoteBookDataSource` on blank API key with a `Timber.e` log. `FallbackRemoteBookDataSource.shouldFallback` includes it. The graceful-degradation path is what makes Crashlytics a hard gate in Phase 2.1.
- [ ] Google Books API key is sent via the `X-Goog-Api-Key` header in `GoogleBooksApiService`, **not** via a `?key=…` URL parameter. `HttpClientFactory.redactSensitiveValues` strips credential values from both URL params and sensitive headers in Ktor log output. This is the credential-exposure fix that must be in place before any APK reaches a tester device.
- [ ] `previewLink` / `infoLink` are HTTPS-coerced in `GoogleBookMappers.toBook` (non-HTTPS → `null`) and launched via the `openExternalUrl` helper with a scheme allowlist. Phase 2.2 verifies this is still in place.
- [ ] Room database is at `version = 1` with no `fallbackToDestructiveMigration` in `DatabaseFactory`. `app/schemas/.../MyBookshelfRoomDatabase/` contains exactly `1.json`. Phase 4.1 depends on this for the tester-uninstall messaging.
- [ ] Description-fetch flow uses targeted `BookshelfDao.updateDescription` (not `upsertBook`), guarded by `book.description.isNullOrBlank()` so the fetch doesn't fire on every detail-screen open. Phase 4.2 quota sanity depends on this.

## Phase 1 — Preventative checks

### 1.1 Koin module `verify()` test

**Why.** On 2026-05-26 the app crashed on cold launch after an `apiKeyProvider: () -> String = { ApiConfig.GoogleBooks.apiKey }` test seam was added to `GoogleBooksApiService` and `GoogleBooksRemoteBookDataSource`, both wired via `singleOf(::Ctor)` in `BookModule`. Koin's `singleOf` ignores Kotlin default-parameter values and tries to resolve every constructor parameter from the graph — the unregistered `() -> String` blew up at runtime. All 685 unit tests passed (they construct these classes directly, never through Koin); only an actual launch surfaced it. The fix was to switch the two registrations to explicit `single { Ctor(arg = get()) }` so the Kotlin default applies — see commit `68c6c617` on `spike-test-google-books` (or its merge commit on `main`).
...
The graceful-degradation path (see Prerequisites): blank/revoked key short-circuits to `PROVIDER_UNAVAILABLE`, `FallbackRemoteBookDataSource` includes that in `shouldFallback`, OL serves the user, and the only signal that something is wrong is a `Timber.e` log. Tester devices are not accessible for logcat — so without remote crash reporting, blank-key bugs are completely silent in the field.
...
`GoogleBooksRemoteBookDataSource` logs at `Timber.tag(TAG).e(...)` on blank key. Add a Timber `Tree` (e.g. `CrashlyticsTree`) installed only in the release build type that forwards `Log.ERROR` priority to `FirebaseCrashlytics.recordException(...)`. Pattern is standard; see Crashlytics docs.
...
This is a verification step — the mapper-level HTTPS coercion and launch-site scheme allowlist were both delivered on the spike branch. Confirm they survived the merge before promoting to closed testing:

- Inspect `GoogleBookMappers.toBook` and confirm non-HTTPS `previewLink` / `infoLink` are dropped to `null`.
- Inspect `PublicationDetailsCard.kt` and confirm `uriHandler.openUri(url)` is gated behind a scheme allowlist (typically `url.startsWith("https://")` plus an optional host check) via the `openExternalUrl` helper.
- Confirm `GoogleBookMappersTest` includes test cases for hostile schemes (`mailto:`, `intent://`, `javascript:`, etc.).

If any of these are missing, that's a regression from what landed on the branch — block promotion and restore.
...
**Note on verifying the key is present in a release APK:** the key is sent via the `X-Goog-Api-Key` request header (see Prerequisites), **not** as a `?key=…` URL parameter. Grepping the built APK for `?key=` or scanning URL strings will return nothing — that does **not** mean the key is missing. The only places the key now appears in the APK are:
...
If 429s spike, the description-fetch guard (see Prerequisites: `book.description.isNullOrBlank()` gate in `BookDetailViewModel`) should have eliminated redundant fetches — confirm it's still in place. Otherwise, consider raising the quota or trimming test cohort size before broadening.

## Definition of done

Promotion to closed testing is gated on **all** of the following:

- [ ] Schema reset to v1 has merged on `main`; verified `version = 1` and `fallbackToDestructiveMigration` removed (see Prerequisites).
...
- [ ] 2.1 — Crashlytics integrated end-to-end (plugin, dep, release Firebase project, `google-services.json`, `CrashlyticsTree`). Required because the blank-key path is graceful-degradation: `PROVIDER_UNAVAILABLE` log is the only signal a blank/revoked key is in production. Also captures Compose runtime crashes, Room failures, Firebase auth issues, and anything else release-build testers hit.
- [ ] 2.2 — `previewLink` / `infoLink` mapper coercion + launch-site allowlist verified present on `main`.
