...
class CreateShelfUseCaseTest {
    ...
    private val mockGetOrCreateTutorialBook =
        object : GetOrCreateTutorialBookUseCase {
            override suspend fun execute(tutorialShelfId: String): Result<String, DataError.Local> {
                return Result.Success("tutorial-book-id")
            }
        }
    private val useCase =
        CreateShelfUseCaseImpl(mockRepository, testIdGenerator, mockGetOrCreateTutorialBook, mockSyncSchedulerService)

    @Test
    fun `creates shelf with correct data when no existing shelves`() =
        runTest {
            // Given
            val name = "My Books"
    ...
    @Test
    fun `calculates correct position when existing shelves present`() =
        runTest {
            // Given
            val existingShelves =
                listOf(
    ...
    @Test
    fun `generates unique ID for each shelf`() =
        runTest {
