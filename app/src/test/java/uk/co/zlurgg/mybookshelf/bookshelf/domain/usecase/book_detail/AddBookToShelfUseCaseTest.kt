...
class AddBookToShelfUseCaseTest {
    ...
    private fun setUpDefaultShelf(shelfId: String) {
        mockBookcaseRepository.shelfByIdToReturn =
            TestShelfBuilder()
                .withId(shelfId)
                .withName("Test Shelf")
                .withBooks(emptyList())
    ...
    @Test
    fun `execute successfully adds book to shelf`() =
        runTest {
            // Given
            val book =
                TestBookBuilder()
    ...
    @Test
    fun `execute creates proper shelf-book relationship`() =
        runTest {
            // Given
            val book =
                TestBookBuilder()
    ...
    @Test
    fun `execute with existing book updates book data but preserves personal metadata`() =
        runTest {
    ...
    @Test
    fun `execute can add same book to multiple shelves`() =
        runTest {
            // Given
            val book =
                TestBookBuilder()
    ...
    @Test
    fun `execute returns error when book repository fails`() =
        runTest {
            // Given
    ...
    @Test
    fun `execute returns error when bookshelf repository fails`() =
        runTest {
            // Given
            val book = TestBookBuilder().withId("test-book").build()
            val shelfId = "test-shelf"
    ...
    @Test
    fun `execute handles empty book ID gracefully`() =
        runTest {
            // Given
            val book =
                TestBookBuilder()
    ...
    @Test
    fun `execute handles empty shelf ID gracefully`() =
        runTest {
            // Given
            val book = TestBookBuilder().withId("test-book").build()
    ...
    @Test
    fun `execute persists complete book data`() =
        runTest {
            // Given
    ...
    @Test
    fun `execute preserves personal metadata when book already exists`() =
        runTest {
    ...
    @Test
    fun `execute uses API data as-is for new books`() =
        runTest {
    ...
    @Test
    fun `execute returns MAX_BOOKS_REACHED when shelf has 20 books`() =
        runTest {
            // Given - Shelf already at max capacity
            val existingBooks =
                (1..AddBookToShelfUseCaseImpl.MAX_BOOKS_PER_SHELF).map { i ->
                    TestBookBuilder()
                        .withId("book-$i")
                        .withTitle("Book $i")
                        .build()
    ...
    @Test
    fun `execute adds book successfully when shelf has fewer than 20 books`() =
        runTest {
    ...
    @Test
    fun `execute adds 20th book successfully at the limit boundary`() =
        runTest {
    ...
    @Test
    fun `execute returns NOT_FOUND when shelf does not exist`() =
        runTest {
