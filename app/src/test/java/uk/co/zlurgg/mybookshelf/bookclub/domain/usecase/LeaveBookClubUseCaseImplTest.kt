...
class LeaveBookClubUseCaseImplTest {
    ...
    @Test
    fun `returns DOCUMENT_NOT_FOUND when shelf does not exist`() = runTest {
        // Given
    ...
    @Test
    fun `returns CLUB_NOT_FOUND when shelf is not a book club`() = runTest {
        // Given - regular shelf, not a book club
        val regularShelf = TestShelfBuilder()
    ...
    @Test
    fun `returns CLUB_NOT_FOUND when book club shelf has null club code`() = runTest {
        // Given - book club shelf with missing club code (edge case)
        val bookClubShelfNoCode = TestShelfBuilder()
    ...
    @Test
    fun `returns success when user successfully leaves book club`() = runTest {
        // Given - valid book club shelf
        val bookClubShelf = TestShelfBuilder()
            .withId("club-shelf")
    ...
    @Test
    fun `calls repository with correct club code`() = runTest {
        // Given
        val bookClubShelf = TestShelfBuilder()
    ...
    @Test
    fun `returns CREATOR_CANNOT_LEAVE when user is creator`() = runTest {
    ...
    @Test
    fun `propagates NOT_SIGNED_IN error from repository`() = runTest {
        // Given
        val bookClubShelf = TestShelfBuilder()
            .withId("club-shelf")
            .withName("Test Club")
    ...
    @Test
    fun `propagates NETWORK_ERROR from repository`() = runTest {
        // Given
        val bookClubShelf = TestShelfBuilder()
            .withId("club-shelf")
            .withName("Test Club")
    ...
    @Test
    fun `propagates NOT_MEMBER error from repository`() = runTest {
        // Given
        val bookClubShelf = TestShelfBuilder()
            .withId("club-shelf")
            .withName("Test Club")
