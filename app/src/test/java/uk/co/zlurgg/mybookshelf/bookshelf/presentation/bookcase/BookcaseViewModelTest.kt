...
class BookcaseViewModelTest {
    ...
    private fun createViewModel(): BookcaseViewModel {
        ...
        val mockHandleTutorialAccess =
            object : HandleTutorialAccessUseCase {
        ...
        val mockCheckForUpdate =
            object : CheckForUpdateUseCase {
                override suspend fun invoke(forceCheck: Boolean): UpdateInfo? = null
            }
        val mockDownloadUpdate =
            object : DownloadUpdateUseCase {
                override fun invoke(updateInfo: UpdateInfo): Long? = null
            }
        val mockDismissUpdate =
            object : DismissUpdateUseCase {
                override suspend fun invoke(version: String) {}
            }
        val mockGetCurrentVersionInfo =
            object : GetCurrentVersionInfoUseCase {
                override suspend fun invoke(): UpdateInfo? = null
            }
        val mockAuthService =
            object : uk.co.zlurgg.mybookshelf.auth.domain.service.AuthService {
        ...
        val mockAuthStateRepository =
            object : uk.co.zlurgg.mybookshelf.auth.domain.repository.AuthStateRepository {
                override suspend fun isSignedIn() = false

                override suspend fun setSignedInState(isSignedIn: Boolean) {}
            }
        val mockSyncScheduler =
        ...
        val mockCreateBookClub =
            object : CreateBookClubUseCase {
                override suspend fun execute(shelfId: String): Result<String, DataError.Sync> =
                    Result.Success("ABC12345")
            }
        val mockGenerateInviteLink =
            object : GenerateInviteLinkUseCase {
                override fun execute(
                    clubCode: String,
                    clubName: String?,
                ): String = "https://mybookshelf.app/join/$clubCode"
            }
        val mockParseClubCode =
            object : ParseClubCodeUseCase {
                override fun invoke(input: String): Result<String, DataError.Validation> = Result.Success("TESTCODE")
            }
        val mockGetBookClubPreview =
            object : GetBookClubPreviewUseCase {
                override suspend fun invoke(code: String): Result<BookClub?, DataError.Sync> = Result.Success(null)
            }
        val mockJoinBookClub =
            object : JoinBookClubUseCase {
        ...
        val mockRestoreBookClubMemberships =
        ...
        val mockSyncBookClub =
            object : SyncBookClubUseCase {
                override suspend fun execute(
                    clubCode: String,
                    localShelfId: String,
                ): Result<SyncResult, DataError.Sync> = Result.Success(SyncResult(0, 0))
            }
        val mockLeaveBookClub =
            object : LeaveBookClubUseCase {
                override suspend fun invoke(shelfId: String): Result<Unit, DataError.Sync> = Result.Success(Unit)
            }
        val mockValidateMemberships =
            object : ValidateBookClubMembershipsUseCase {
                override suspend fun invoke(): Result<List<String>, DataError.Sync> = Result.Success(emptyList())
            }
        val mockGetBookClubReviews =
            object : GetBookClubReviewsUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                ): Result<List<BookClubReview>, DataError.Sync> = Result.Success(emptyList())
            }
        val mockUpsertBookClubReview =
            object : UpsertBookClubReviewUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                    rating: Float,
        ...
        val mockDeleteBookClubReview =
            object : DeleteBookClubReviewUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                ): Result<Unit, DataError.Sync> = Result.Success(Unit)
            }
        val mockGetBookClubComments =
            object : GetBookClubCommentsUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                ): Result<List<BookClubComment>, DataError.Sync> = Result.Success(emptyList())
            }
        val mockAddBookClubComment =
            object : AddBookClubCommentUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                    text: String,
        ...
        val mockEditBookClubComment =
            object : EditBookClubCommentUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                    commentId: String,
        ...
        val mockDeleteBookClubComment =
            object : DeleteBookClubCommentUseCase {
                override suspend fun invoke(
                    clubCode: String,
                    bookId: String,
                    commentId: String,
    ...
    @Test
    fun `ShowAddDialog action toggles dialog visibility`() =
    ...
    @Test
    fun `ToggleReorderMode changes reorder state`() =
        runTest(testDispatcher) {
            // Given
            val viewModel = createViewModel()
            val stateHelper = viewModel.state.testHelper(this)
    ...
    @Test
    fun `ResetOperationState clears error and success flags`() =
        runTest(testDispatcher) {
            // Given
            val viewModel = createViewModel()
            val stateHelper = viewModel.state.testHelper(this)
    ...
    @Test
    fun `delete shelf handles error correctly`() =
        runTest(testDispatcher) {
            // Given
    ...
    @Test
    fun `restore shelf handles error correctly`() =
        runTest(testDispatcher) {
            // Given
            val testShelf = TestShelfBuilder().withId("shelf-1").withName("Test Shelf").build()
            val bookcase = Bookcase(id = "bookcase", bookshelves = listOf(testShelf), bookCounts = emptyMap())
    ...
    @Test
    fun `add shelf handles error correctly`() =
        runTest(testDispatcher) {
            // Given
    ...
    @Test
    fun `ShowRenameDialog action sets shelf to rename and shows dialog`() =
        runTest(testDispatcher) {
            // Given
            val testShelf = TestShelfBuilder().withId("shelf-1").withName("Test Shelf").build()
            val viewModel = createViewModel()
    ...
    @Test
    fun `DismissRenameDialog action hides dialog and clears shelf`() =
        runTest(testDispatcher) {
            // Given
            val testShelf = TestShelfBuilder().withId("shelf-1").withName("Test Shelf").build()
            val viewModel = createViewModel()
    ...
    @Test
    fun `rename shelf success updates shelf name in list`() =
        runTest(testDispatcher) {
            // Given
    ...
    @Test
    fun `rename shelf handles error correctly`() =
        runTest(testDispatcher) {
            // Given
            val testShelf = TestShelfBuilder().withId("shelf-1").withName("Old Name").build()
            val bookcase = Bookcase(id = "bookcase", bookshelves = listOf(testShelf), bookCounts = emptyMap())
    ...
    @Test
    fun `dismissing rename dialog clears inline error`() =
        runTest(testDispatcher) {
            // Given
            val testShelf = TestShelfBuilder().withId("shelf-1").withName("Old Name").build()
            val bookcase = Bookcase(id = "bookcase", bookshelves = listOf(testShelf), bookCounts = emptyMap())
