...
/**
 * E2E test for book removal workflow.
...
class BookRemovalE2ETest {
    ...
    fun removeBookUpdatesStateAndPersistsToDatabase() = runBlocking {
        ...
        assertEquals("Book Two", state.books[0].title)
        assertNotNull(state.recentlyDeleted)
    ...
    fun undoRemoveRestoresBookToShelf() = runBlocking {
        ...
        assertEquals(2, stateAfterUndo.books.size)
    ...
    fun removeBookOnlyRemovesFromCurrentShelf() = runBlocking {
        ...
        // Add book-1 to both shelves
        bookshelfRepository.addBookToShelf(anotherShelfId, "book-1")

        // When - User removes book from test shelf
        val state = bookshelfViewModel.state.first()
